		...
		uint8_t num_base_bdev_to_use)
{
	...
	r->base_bdevs.count = num_base_bdev_to_use;
...
static void
_create_raid_bdev_create_req(struct rpc_bdev_raid_create_ctx *r, const char *raid_name,
...
test_raid_grow_base_bdev(void)
{
	...
	_create_raid_bdev_create_req(&req, "raid1", 0, true, 0, false,
				     g_max_base_drives - 2);
	req.strip_size_kb = 0;
	req.raid_level = (enum rpc_bdev_raid_level)SPDK_BDEV_RAID_LEVEL_RAID1;
	rpc_bdev_raid_create(NULL, NULL);
	CU_ASSERT(g_rpc_err == 0);
	verify_raid_bdev(&req, true, SPDK_BDEV_RAID_STATE_ONLINE);
	...
	pbdev->state = SPDK_BDEV_RAID_STATE_CONFIGURING;
	rc = raid_bdev_grow_base_bdev(pbdev, name1, NULL, NULL);
	CU_ASSERT(rc == -EINVAL);
	pbdev->state = SPDK_BDEV_RAID_STATE_ONLINE;
